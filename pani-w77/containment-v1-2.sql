...
begin
 ...
 if active_event is not null and phase is not null then
   ...
   if (active_event='death' and role_name='witness') or (active_event in ('knowledge','energy','blood') and role_name='representative') then
...
begin
 ...
 if p_action='vote' then
   ...
   selected:=coalesce(p_payload->'words','[]'); if jsonb_typeof(selected)<>'array' or jsonb_array_length(selected)<>4 or (select count(distinct x) from jsonb_array_elements_text(selected)x)<>4 then raise exception 'four_unique_words_required'; end if;
   if not (ev->'words' @> selected) then raise exception 'invalid_group'; end if;
   groups:=sec#>'{knowledge,solution}'; solved:=coalesce(ev->'solved','[]');
   ...
   if matched is not null and not (solved @> jsonb_build_array(matched->>'name')) then
     solved:=solved||jsonb_build_array(matched->>'name'); ev:=ev||jsonb_build_object('solved',solved,'solved_groups',coalesce(ev->'solved_groups','[]')||jsonb_build_array(matched));
   ...
   if active_event<>'knowledge' or c.crew_id<>rep or ev->>'phase'<>'final' then raise exception 'representative_required'; end if;st:=pani_private.containment_capture(st,'knowledge');
   ...
   if value=answer then cycle:=coalesce((ev->>'cycle')::int,1)+1;if cycle>4 then ev:=ev||jsonb_build_object('phase','final','active_side','PLAYERS','cycle',4,'vote_phase','death:hold');st:=st||jsonb_build_object('witness_id',null);else ev:=ev||jsonb_build_object('cycle',cycle,'phase','master_prepare','active_side','MASTER','changed_visuals','[]','vote_phase',null,'previous_witness',witness);sec:=jsonb_set(sec,'{death}',jsonb_build_object('cycle',cycle,'answer',null,'choices',pani_private.containment_death_choices(cycle,ev->>'anchor')),true);st:=jsonb_set(st,'{witness_id}','null',true);end if;else ev:=ev||jsonb_build_object('attempts',coalesce((ev->>'attempts')::int,0)+1);st:=jsonb_set(st,'{saturation}',to_jsonb(least(8,coalesce((st->>'saturation')::int,0)+1)));end if;
...
begin
 ...
 if p_action='start_event' then
   ...
   if target in('knowledge','energy','blood') and not exists(select 1 from pani_private.containment_participant p where p.session_id='W77-01' and p.crew_id=rep and p.joined) then raise exception 'representative_unavailable';end if;if target='death' then rep:=null;end if;
   ...
   if target='knowledge' then ev:=jsonb_build_object('phase','groups','active_side','PLAYERS','words',pani_private.containment_conexo_words(),'solved','[]','solved_groups','[]','coherence',4,'partial',false,'hint',null,'vote_phase','knowledge:g1');sec:=jsonb_set(sec,'{knowledge}',jsonb_build_object('solution',pani_private.containment_conexo_solution(),'corruption_hand',jsonb_build_array('shuffle','ghost_connection','unstable_word','index_block')),true);
   elsif target='energy' then deck:=jsonb_build_array('ZUMBI DE SANGUE','ESQUELETO DE LODO','ANÁRQUICO','EXISTIDO','ENRAIZADO','MARIONETE','VIAJANTE','CARNIÇAL PRETO DA MORTE','ENPAP-X','DEGOLIFICADA','MAGISTRADA','DEUS DA MORTE','DIABO','ANFITRIÃO');ev:=jsonb_build_object('phase','trail','active_side','PLAYERS','position',0,'overload',0,'pulse',1+floor(random()*4)::int,'turn',1,'checkpoints',jsonb_build_array(6,13),'forks',jsonb_build_array(3,9,15),'last_card',null,'last_effect','Aguardando pulso.','vote_phase','energy:t1');sec:=jsonb_set(sec,'{energy}',jsonb_build_object('deck',deck,'index',0,'hand','[]'),true);
   elsif target='blood' then seconds:=greatest(12,least(18,coalesce((p_payload->>'beat_seconds')::int,15)));ev:=jsonb_build_object('phase','master_prepare','active_side','MASTER','beat',1,'beat_seconds',seconds,'stabilization',0,'hemorrhage',0,'readings','[]','selected_valve',null,'vote_phase',null);sec:=jsonb_set(sec,'{blood}',jsonb_build_object('beat_index',1,'answer',null,'answer_locked',false,'choices',pani_private.containment_blood_choices(1),'fake_votes',null),true);
   else ev:=jsonb_build_object('phase','master_prepare','active_side','MASTER','cycle',1,'anchor',null,'attempts',0,'changed_visuals','[]','vote_phase',null);sec:=jsonb_set(sec,'{death}',jsonb_build_object('cycle',1,'answer',null,'choices',pani_private.containment_death_choices(1,null)),true);end if;
   st:=jsonb_set(st,'{event}',ev,true);
 elsif p_action='set_representative' then rep:=coalesce(p_payload->>'crew_id','');if st->>'active_event' not in('knowledge','energy','blood') then raise exception 'representative_not_required';end if;if not exists(select 1 from pani_private.containment_participant p where p.session_id='W77-01' and p.crew_id=rep and p.joined) then raise exception 'representative_unavailable';end if;st:=jsonb_set(st,'{representative_id}',to_jsonb(rep),true);
