-- ADUNATIO // PROTOCOLO PANI // EVENTOS DE CONTENCAO v1.3
...
declare c public.crew_access%rowtype; s pani_private.containment_session%rowtype; st jsonb; sec jsonb; ev jsonb;
 ...
 coherence int; pos int; pulse int; overload int; cycle int; answer text; choices jsonb; control_mode text;
begin
 ...
 if p_action='vote' then
   ...
   if active_event<>'knowledge' or ev->>'phase'<>'final' then raise exception 'knowledge_unavailable'; end if;st:=pani_private.containment_capture(st,'knowledge');ev:='{}'::jsonb;
 elsif p_action='energy_move' then
   control_mode:=coalesce(ev->>'control_mode','operator');
   if active_event<>'energy' or ev->>'phase'<>'race' then raise exception 'energy_move_unavailable';end if;
   if control_mode='operator' and c.crew_id<>rep then raise exception 'representative_required';end if;
   pos:=coalesce((ev->>'player_position')::int,(ev->>'position')::int,0);pulse:=greatest(1,least(6,coalesce((ev->>'pulse')::int,1)));overload:=coalesce((ev->>'overload')::int,0);value:=coalesce(p_payload->>'route','advance');
   ...
   pos:=least(19,pos+pulse);if pos>=19 then ev:=ev||jsonb_build_object('phase','finished','active_side','PLAYERS','winner','PLAYERS','player_position',19,'position',19,'vote_phase',null,'last_effect','A Equipe alcançou a contenção primeiro.');else
     ...
     sec:=jsonb_set(sec,'{energy,hand}',choices,true);ev:=ev||jsonb_build_object('player_position',pos,'position',pos,'overload',overload,'active_side','MASTER','phase','master_threat','master_pulse',1+floor(random()*4)::int,'vote_phase',null,'last_effect','O Mestre seleciona uma ameaça e move a Criatura.');end if;
   delete from pani_private.containment_vote where session_id='W77-01' and event_id='energy';
 elsif p_action='energy_claim' then
   control_mode:=coalesce(ev->>'control_mode','operator');
   if active_event<>'energy' or ev->>'phase'<>'finished' or ev->>'winner'<>'PLAYERS' then raise exception 'energy_claim_unavailable';end if;
   if control_mode='operator' and c.crew_id<>rep then raise exception 'representative_required';end if;
   st:=pani_private.containment_capture(st,'energy');ev:='{}'::jsonb;delete from pani_private.containment_vote where session_id='W77-01' and event_id='energy';
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare s pani_private.containment_session%rowtype;st jsonb;sec jsonb;ev jsonb;target text;rep text;statuses jsonb;deck jsonb;idx int;card text;effect text;pos int;master_pos int;master_pulse int;overload int;choice jsonb;beat int;seconds int;cycle int;answer text;control_mode text;
begin
 ...
 if p_action='start_event' then
   ...
   control_mode:=case when target='energy' then lower(coalesce(p_payload->>'control_mode','team')) else 'operator' end;if control_mode not in('team','operator') then raise exception 'invalid_control_mode';end if;
   if (target='blood' or (target='energy' and control_mode='operator')) and not exists(select 1 from pani_private.containment_participant p where p.session_id='W77-01' and p.crew_id=rep and p.joined) then raise exception 'representative_unavailable';end if;if target in('knowledge','death') or (target='energy' and control_mode='team') then rep:=null;end if;
   statuses:=jsonb_set(st->'event_status',array[target],'"in_progress"',true);st:=st||jsonb_build_object('protocol_version','containment_v1_3','status','active','active_event',target,'event_status',statuses,'representative_id',rep,'witness_id',null,'paused',false,'timer_end',null,'corruption_charges',2);
   ...
   elsif target='energy' then deck:=jsonb_build_array('ZUMBI DE SANGUE','ESQUELETO DE LODO','ANÁRQUICO','EXISTIDO','ENRAIZADO','MARIONETE','VIAJANTE','CARNIÇAL PRETO DA MORTE','ENPAP-X','DEGOLIFICADA','MAGISTRADA','DEUS DA MORTE','DIABO','ANFITRIÃO');ev:=jsonb_build_object('phase','race','active_side','PLAYERS','control_mode',control_mode,'player_position',0,'master_position',0,'position',0,'winner',null,'overload',0,'pulse',1+floor(random()*4)::int,'master_pulse',1+floor(random()*4)::int,'turn',1,'master_wins',0,'checkpoints',jsonb_build_array(6,13),'forks',jsonb_build_array(3,9,15),'last_card',null,'last_effect','Equipe e Criatura aguardam o primeiro pulso.','vote_phase','energy:t1');sec:=jsonb_set(sec,'{energy}',jsonb_build_object('deck',deck,'index',0,'hand','[]'::jsonb),true);
   ...
   if st->>'active_event'<>'energy' or ev->>'active_side'<>'MASTER' or ev->>'phase'<>'master_threat' then raise exception 'players_turn';end if;
   card:=upper(coalesce(p_payload->>'card','PASSAR'));if card<>'PASSAR' and not (sec#>'{energy,hand}' @> jsonb_build_array(card)) then raise exception 'invalid_threat';end if;
   pos:=coalesce((ev->>'player_position')::int,(ev->>'position')::int,0);master_pos:=coalesce((ev->>'master_position')::int,0);master_pulse:=greatest(1,least(6,coalesce((ev->>'master_pulse')::int,1)));overload:=coalesce((ev->>'overload')::int,0);effect:='O Mestre passou a ameaça e moveu a Criatura.';
   if card='ZUMBI DE SANGUE' then pos:=greatest(0,pos-2);effect:='ZUMBI DE SANGUE: Equipe recua 2.';elsif card='ESQUELETO DE LODO' then pos:=greatest(0,pos-1);effect:='ESQUELETO DE LODO: Equipe recua 1.';elsif card='ANÁRQUICO' then pos:=greatest(0,pos-2);effect:='ANÁRQUICO: Equipe recua 2.';elsif card in('EXISTIDO','ENRAIZADO','MARIONETE','VIAJANTE') then overload:=overload+1;effect:=card||': +1 sobrecarga para a Equipe.';elsif card in('CARNIÇAL PRETO DA MORTE','DIABO') then overload:=overload+2;pos:=greatest(0,pos-1);effect:=card||': +2 sobrecarga e Equipe recua 1.';elsif card<>'PASSAR' then overload:=overload+1;effect:=card||': +1 sobrecarga para a Equipe.';end if;
   if overload>=7 then pos:=case when pos>=13 then 13 when pos>=6 then 6 else 0 end;overload:=4;st:=jsonb_set(st,'{saturation}',to_jsonb(least(8,coalesce((st->>'saturation')::int,0)+1)));effect:=effect||' Sobrecarga crítica: Equipe retorna ao checkpoint.';end if;
   master_pos:=least(19,master_pos+master_pulse);idx:=coalesce((sec#>>'{energy,index}')::int,0)+3;sec:=jsonb_set(sec,'{energy,index}',to_jsonb(idx),true);sec:=jsonb_set(sec,'{energy,hand}','[]'::jsonb,true);
   if master_pos>=19 then ev:=ev||jsonb_build_object('phase','finished','active_side','MASTER','winner','MASTER','player_position',pos,'position',pos,'master_position',19,'overload',overload,'last_card',card,'last_effect',effect||' A Criatura alcançou a contenção primeiro.','vote_phase',null);
   else ev:=ev||jsonb_build_object('phase','race','active_side','PLAYERS','winner',null,'player_position',pos,'position',pos,'master_position',master_pos,'overload',overload,'pulse',1+floor(random()*4)::int,'master_pulse',1+floor(random()*4)::int,'turn',coalesce((ev->>'turn')::int,1)+1,'last_card',card,'last_effect',effect,'vote_phase','energy:t'||(coalesce((ev->>'turn')::int,1)+1));end if;
 elsif p_action='energy_rematch' then
   if st->>'active_event'<>'energy' or ev->>'phase'<>'finished' or ev->>'winner'<>'MASTER' then raise exception 'rematch_unavailable';end if;
   ev:=ev||jsonb_build_object('phase','race','active_side','PLAYERS','winner',null,'player_position',0,'position',0,'master_position',0,'overload',0,'pulse',1+floor(random()*4)::int,'master_pulse',1+floor(random()*4)::int,'turn',1,'master_wins',coalesce((ev->>'master_wins')::int,0)+1,'last_card',null,'last_effect','Revanche iniciada pelo Mestre.','vote_phase','energy:t1');sec:=jsonb_set(sec,'{energy,hand}','[]'::jsonb,true);
...
comment on function public.pani_containment_crew_action_v12(text,text,jsonb) is 'Acoes da equipe v1.3 com Energia em corrida, Conexo e validacao server-side.';
comment on function public.pani_containment_master_action_v12(text,text,jsonb) is 'Acoes adversariais exclusivas do Mestre v1.3, incluindo peca e vitoria da Criatura em Energia.';
