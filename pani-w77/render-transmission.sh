...
version=(os.environ.get('RENDER_GIT_COMMIT') or 'pani-v13')[:12]
...
for needle in ('GLIFOS CANÔNICOS v13','TX_ALPHA_W=1280','TX_ALPHA_H=800','TX_ALPHA_CELL=160','TX_ALPHA_CROPS','txBuildIndependentAlphabet','canvas','toDataURL','<svg class=','viewBox=','tx-alpha-independent','alphabet-v13','txShowPlayerSignal=function','txRenderSelected=function'):
    assert needle in fix,needle
for forbidden in ('alpha-img-v11','left:-${s.col*100}%','background-position:${s.pos}'):
    assert forbidden not in fix,forbidden

css=Path('public/transmission-sprites.css').read_text(encoding='utf-8')
for needle in ('GLIFOS CANÔNICOS EMPACOTADOS v13','tx-alpha-viewport','tx-alpha-svg','tx-alpha-independent'):
...
print(f'PANI final production audit OK // ALPHABET-REDUNDANT v13 // {version}')
PY
