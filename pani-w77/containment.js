...
const CT_EVENT_META={
  knowledge:{title:'PROTOCOLO TRÍADE',short:'CONHECIMENTO',module:'ÍNDICE & ACESSO',gravity:'ALTA',icon:'◇'},
...
function ctRoleBanner(){let role=ctRole(),name=role==='representative'?'REPRESENTANTE':role==='witness'?'TESTEMUNHA':'EQUIPE / OBSERVADOR';return `<div class="ct-role ${role}"><span>FUNÇÃO ATUAL</span><b>${name}</b><small>${role==='representative'?'Sua confirmação altera o estado da arena.':role==='witness'?'Observe o consenso e confirme o quadro real.':'Vote livremente; sua recomendação não substitui a decisão final.'}</small></div>`}

function containmentKnowledge(){let ev=ctEvent(),size=ctSafeInt(ev.size,3),marks=ev.marks||{},unstable=new Set(ev.unstable||[]),absent=new Set(ev.absent||[]),ghost=new Set(ev.ghost_cells||[]),summary=containmentState?.vote_summary||{};
  let cells=Array.from({length:size*size},(_,i)=>{let id='c'+i,isAbsent=absent.has(id),vote=ctSafeInt(summary[id]),canAct=ctIsRepresentative();return `<button class="ct-kcell ${unstable.has(id)?'unstable':''} ${isAbsent?'absent':''} ${ghost.has(id)?'ghost':''} ${containmentState.my_vote===id?'voted':''}" ${marks[id]||(!isAbsent&&ev.phase==='missing')?'disabled':''} onclick="${canAct?(isAbsent?`containmentAction('knowledge_missing',{cell:'${id}'})`:`containmentAction('knowledge_mark',{cell:'${id}'})`):`containmentVote('${id}')`}"><span>${marks[id]||''}</span>${isAbsent?'<b>CASA<br>AUSENTE</b>':''}${vote?`<small>${vote} voto${vote===1?'':'s'}</small>`:''}</button>`}).join('');
  return `<div class="card ct-shell knowledge">${ctHeader()}${ctRoleBanner()}<div class="ct-arena-head"><div><div class="k">CONHECIMENTO // RODADA ${ev.round||1}/3</div><h2>${esc(ev.rule_title||'PROTOCOLO TRÍADE')}</h2><p>${esc(ev.rule_text||'Aguardando regra.')}</p>${ev.hint?`<div class="ct-hint">${esc(ev.hint)}</div>`:''}</div><div class="ct-score"><b>${ev.wins||0}</b><span>RODADAS VENCIDAS</span></div></div><div class="ct-kboard size${size}">${cells}</div>${ctIsRepresentative()?`<div class="actions"><button class="btn" onclick="containmentAction('knowledge_scan')" ${ctState()?.pani_charges?.scan<1?'disabled':''}>SCAN // ${ctState()?.pani_charges?.scan||0}</button><button class="btn" onclick="containmentAction('knowledge_lock')" ${ctState()?.pani_charges?.lock<1?'disabled':''}>LOCK // ${ctState()?.pani_charges?.lock||0}</button></div>`:'<p class="mut small">Selecione uma casa para recomendar a jogada ao representante.</p>'}${ctIsRepresentative()?ctConsensus(Array.from({length:size*size},(_,i)=>({value:'c'+i,label:'CASA '+(i+1)}))):''}</div>`
}
...
function containmentMasterStart(event){let representative_id=$('#ctmrep')?.value||'';if(!representative_id)return toast('Escolha um representante conectado.',true);return containmentMasterAction('start_event',{event,representative_id,beat_seconds:ctSafeInt($('#ctmbloodseconds')?.value,15)})}
function containmentMasterRep(){let crew_id=$('#ctmrep')?.value||'';if(crew_id&&ctState().active_event)return containmentMasterAction('set_representative',{crew_id})}
...
function ctMasterSpecific(id){let st=containmentMasterState.state||{},ev=st.event||{},sec=containmentMasterState.secret_state||{};
  ...
  if(id==='death')return `<div class="ctm-specific"><div class="k">MORTE // OBJETO REAL: ${esc(CT_OBJECTS[sec?.death?.answer]||sec?.death?.answer||'—')}</div><label>TESTEMUNHA DO CICLO<select id="ctmwitness" onchange="containmentMasterWitness()">${ctMasterPlayerOptions(st.witness_id||'')}</select></label><div class="actions">${[1,2,3,4].map(n=>`<button class="btn" onclick="containmentMasterAction('death_scene',{cycle:${n}})">CENA ${n}</button>`).join('')}</div></div>`;
  return ''
}
...
function containmentMasterRender(){let root=$('#ctmasterbody');if(!root)return;let data=containmentMasterState,st=data.state||{},released=st.released===true;
  ...
  <label>REPRESENTANTE ATUAL<select id="ctmrep" onchange="containmentMasterRep()">${ctMasterPlayerOptions(st.representative_id||'')}</select></label>
