...
function containmentKnowledge(){let ev=ctEvent(),size=ctSafeInt(ev.size,3),marks=ev.marks||{},unstable=new Set(ev.unstable||[]),absent=new Set(ev.absent||[]),ghost=new Set(ev.ghost_cells||[]),summary=containmentState?.vote_summary||{};
  let myId=containmentState.player?.crew_id,mustAlternate=(containmentState.players||[]).filter(p=>p.connected).length>1&&ev.last_actor_id===myId,cells=Array.from({length:size*size},(_,i)=>{let id='c'+i,isAbsent=absent.has(id);return `<button class="ct-kcell ${unstable.has(id)?'unstable':''} ${isAbsent?'absent':''} ${ghost.has(id)?'ghost':''}" ${mustAlternate||marks[id]||(!isAbsent&&ev.phase==='missing')?'disabled':''} onclick="${isAbsent?`containmentAction('knowledge_missing',{cell:'${id}'})`:`containmentAction('knowledge_mark',{cell:'${id}'})`}"><span>${marks[id]||''}</span>${isAbsent?'<b>CASA<br>AUSENTE</b>':''}</button>`}).join('');
  return `<div class="card ct-shell knowledge">${ctHeader()}${ctRoleBanner()}<div class="ct-arena-head"><div><div class="k">CONHECIMENTO // RODADA ${ev.round||1}/3</div><h2>${esc(ev.rule_title||'PROTOCOLO TRÍADE')}</h2><p>${esc(ev.rule_text||'Aguardando regra.')}</p>${ev.hint?`<div class="ct-hint">${esc(ev.hint)}</div>`:''}</div><div class="ct-score"><b>${ev.wins||0}</b><span>RODADAS VENCIDAS</span></div></div>${mustAlternate?'<div class="ct-corruption-note">AGUARDANDO OUTRO OPERADOR PARA A PRÓXIMA MARCAÇÃO</div>':''}<div class="ct-kboard size${size}">${cells}</div><div class="actions"><button class="btn" onclick="containmentAction('knowledge_scan')" ${ctState()?.pani_charges?.scan<1?'disabled':''}>SCAN // ${ctState()?.pani_charges?.scan||0}</button><button class="btn" onclick="containmentAction('knowledge_lock')" ${ctState()?.pani_charges?.lock<1?'disabled':''}>LOCK // ${ctState()?.pani_charges?.lock||0}</button></div><p class="mut small">Cada clique válido altera imediatamente o tabuleiro compartilhado para toda a equipe.</p></div>`
}
