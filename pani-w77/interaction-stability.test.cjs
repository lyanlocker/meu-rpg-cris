...
class FakeAudioContext{
  constructor(){this.currentTime=10;this.state='running';this.destination={};this.oscillators=[];audioInstances.push(this)}
  resume(){this.state='running';return Promise.resolve()}
  createOscillator(){let o={type:'sine',frequency:{setValueAtTime(v,t){o.frequencyValue=v;o.frequencyTime=t}},connect(){return o},start(t){o.startTime=t},stop(t){o.stopTime=t}};this.oscillators.push(o);return o}
  createGain(){let g={gain:{setValueAtTime(){},exponentialRampToValueAtTime(){}},connect(){return g}};return g}
}
...
const context={URLSearchParams,location:{search:'',pathname:'/'},document,window:{AudioContext:FakeAudioContext},console,setTimeout,clearTimeout,setInterval(){return 1},clearInterval,CustomEvent:class{},performance:{now:()=>1000},Date};
...
vm.runInContext(eco+'\n'+qaPublic+`\nthis.__ecoTest={page:function(id,p,mode){me={crew_id:id,display_name:ECO_NAMES[id]};ecoDraft={};ecoState={eligible:true,released:mode!=='standby',status:mode==='standby'?'dormant':mode,paused:false,coherence:42,locks:mode==='active'?2:5,globalPressure:1,revision:1,convergence:{state:'open',readyCount:3,windowEnd:new Date(Date.now()+11500).toISOString()},sixthLayer:{historicalLayers:6,activeAnchors:5,layer:'ENV'},crossClue:'IFF Ø-C',anchor:{crewId:id,anchorId:ECO_META[id].anchor.toLowerCase(),phase:p,stability:3,progress:(p-1)*25,publicState:qaPublic(id,p),publicLog:[],helpTokens:1,locked:mode!=='active',interference:{},revision:1,output:'ÂNCORA FIXADA'}};return ecoPage()},audio:function(noAudio){ecoAudioContext=null;ecoDraft={audioOff:!!noAudio};ecoState={anchor:{phase:2,publicState:qaPublic('willy',2)}};ecoMedPlay()},masterAnchor:function(paused){return ecoMasterAnchor({crewId:'willy',phase:2,stability:3,progress:25,locked:false,joined:true,helpTokens:0},{status:'active',paused:!!paused,presenceCharges:5})}};`,context);
...
context.__ecoTest.audio(false);
assert.equal(audioInstances.length,1,'reprodução não abriu o contexto de áudio');
assert.equal(audioInstances[0].oscillators.length,5,'fase 2 de Willy não agendou os cinco sinais');
assert.deepEqual(audioInstances[0].oscillators.map(x=>Math.round((x.startTime-10)*1000)),[0,720,1390,2050,2720],'intervalos sonoros divergem da solução');
let audioCount=audioInstances.length;context.__ecoTest.audio(true);
assert.equal(audioInstances.length,audioCount,'modo sem áudio ainda criou contexto sonoro');
assert.match(context.__ecoTest.masterAnchor(false),/RESOLVER FASE/,'controle de resolução do Mestre ausente');
assert.match(context.__ecoTest.masterAnchor(false),/solve_phase/,'controle do Mestre não chama a ação server-authoritative');
assert.match(context.__ecoTest.masterAnchor(true),/disabled[^>]*><span>RESOLVER FASE/,'resolução deve ficar bloqueada durante pausa');
