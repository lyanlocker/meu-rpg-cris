'use strict';
/* PANI W77 // ALFABETO PARANORMAL v17
   CORREÇÃO EXCLUSIVA A-Z / 0-9.

   Diagnóstico confirmado: os WebP embutidos da v15 tinham bytes extras após
   o fim declarado do container RIFF. Navegadores exibiam o ícone nativo de
   imagem quebrada. A v17 usa somente arquivos físicos same-origin reparados
   pelo build: ./glyphs/a01.webp ... ./glyphs/a36.webp

   Sem data URI no DOM, sem atlas em runtime, sem Canvas, sem mask e sem SVG
   para o alfabeto. Os conceitos continuam no pipeline de máscara estável. */

const TX_CONCEPT_ATLAS='./tx-concept-sprite.webp?v=concept-v8';
const TX_ALPHA_VERSION='v17';
...
(function txAlphabetPreflight(){
  ...
  probes.forEach(asset=>{
    const img=new Image();
    img.onload=()=>{ok++;if(ok===probes.length)console.info('PANI TX ALPHABET v17 // REPAIRED SAME-ORIGIN GLYPHS READY')};
    img.onerror=()=>console.error('PANI TX ALPHABET v17 // HTTP GLYPH FAILED',asset,txAlphaSrc(asset));
    img.src=txAlphaSrc(asset);
  });
})();
setTimeout(()=>{try{if(typeof MASTER!=='undefined'&&MASTER){txRenderCatalog();txRenderSelected();txRenderActive()}}catch(e){console.error('PANI alphabet renderer v17',e)}},25);
